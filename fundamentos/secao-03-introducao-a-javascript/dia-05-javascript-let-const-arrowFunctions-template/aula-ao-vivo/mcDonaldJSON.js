...
const moreExpensive = (menu, category, value) => {

    let biggestPrice = menu[category][0][value];

    for (let index = 1; index < menu[category].length; index += 1) {

        (menu[category][index][value] > biggestPrice) && (biggestPrice = menu[category][index][value])

    }

    return ` O produto mais caro é o ${biggestPrice}`;

}

// console.log(moreExpensive(menu, 'bebidas', 'name'));
...
const checkItem = (menu, category, name) => {

    let menuCategory = menu[category];

    for (let index = 0; index < menuCategory.length; index+= 1) {
        
        if (menuCategory[index].name === name) {
            return true;
        }; 
    }
    return false;
...
console.log(
    checkItem(menu, 'bebidas', 'Coca-Cola 300ml')
);

console.log(
    checkItem(menu, 'sobremesas', 'Top Sundae Chocolate')
);
